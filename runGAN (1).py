...
#                 elif opt.NDim==3:
                ...
                if opt.isNetDFullyConv: #if netD is FCN based
                    ...
                    loss_real = criterion_BCE2D(outputD_real,real_label)
                    loss_real.backward()
                    #train with fake data
                    fake_label = Variable(fake_label.cuda())  
                    loss_fake = criterion_BCE2D(outputD_fake,fake_label)
                    loss_fake.backward()      
...
#                 elif opt.NDim==3:
    ...
    #             outputG = outputG.data.max(1)[1]
                ...
                if opt.isNetDFullyConv:       
                    lossG_D = lossG_D + opt.lambdaAD * criterion_BCE2D(outputD, real_label) #note, for generator, the label for outputG is real
                else:
    ...
    #             criterion_dice = myWeightedDiceLoss4Organs(organIDs=[0,1,2,3], organWeights=[1,4,8,6])
                np.save("pred",outputG.cpu().detach().numpy())
                np.save("target",labels.cpu().detach().numpy())
